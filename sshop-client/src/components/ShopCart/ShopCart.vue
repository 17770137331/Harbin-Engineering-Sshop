  ...
  <div>
    <div class="shopcart">
      <div class="content">
        <div class="content-left" @click="toggleShow">
          <div class="logo-wrapper">
            <div class="logo" :class="{ highlight: totalCount }">
              <i
                class="iconfont icon-qicheqianlian-"
        ...
        <div class="content-right" @click="$router.push('/order/confirmorder')" v-if="cartGoods.length">
      ...
      <transition name="move">
        <div class="shopcart-list" v-show="isShow">
          ...
          <div class="list-content">
            <ul>
              <li class="food" v-for="(good, index) in cartGoods" :key="index">
...
export default {
  ...
  watch: {
    ...
    isShow: function () {
      //$nextTick的作用是等到下一个dom生成后再来执行回调函数
      if (this.isShow) {
        this.$nextTick(() => {
          // 实现BScroll的实例是一个单例
          if (!this.scroll) {
  ...
  // 所谓的计算属性，就是值变换之后会引起这些函数的执行
  computed: {
    // 在购物车中获取到cartGoods的state
    ...mapState(["cartGoods"]),
    // 获取相应的Getters里的数据
    ...
    payText() {
      ...
      if (totalPrice === 0) {
        ...
        return `还差￥${minPrice - totalPrice}元包邮`;
      } else {
        return "去结算";
      }
    },
